...
bool DB::ensureTable()
{
    ...
    constexpr const char *CREATE_TABLE_QUERY = "CREATE TABLE IF NOT EXISTS paths ("
                                               "path TEXT PRIMARY KEY, "
                                               "access_count INTEGER NOT NULL, "
                                               "last_accessed INTEGER NOT NULL);";
    char *errMsg = nullptr;
    if (sqlite3_exec(_db, CREATE_TABLE_QUERY, nullptr, nullptr, &errMsg) != SQLITE_OK)
    {
...
bool DB::upsertPath(const std::string &path, std::time_t access_time)
{
    ...
    constexpr const char *UPSERT_PATH_QUERY = "INSERT INTO paths (path, access_count, last_accessed) "
                                              "VALUES (?, 1, ?) "
    ...
    if (sqlite3_prepare_v2(_db, UPSERT_PATH_QUERY, -1, &stmt, nullptr) != SQLITE_OK)
    {
...
bool DB::removePath(const std::string &path)
{
    ...
    sqlite3_stmt *stmt = nullptr;
    if (sqlite3_prepare_v2(_db, REMOVE_PATH_QUERY, -1, &stmt, nullptr) != SQLITE_OK)
    {
        std::cerr << "Failed to prepare statement: " << sqlite3_errmsg(_db) << std::endl;
        return false;
    }
...
std::vector<DB::PathEntry> DB::getPaths() const
{
    ...
    constexpr const char *GET_PATHS_QUERY = "SELECT path, access_count, last_accessed FROM paths;";
    ...
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW)
    {
        ...
        entry.last_accessed = static_cast<std::time_t>(sqlite3_column_int64(stmt, 2));
